...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for ATM A1.");
    info!(log, "Total Duration for ATM A1: {:?}", total_duration);
    println!(
        "Total Duration for ATM A1 .cf file generator: {:2?}",
        total_duration
    );
}
