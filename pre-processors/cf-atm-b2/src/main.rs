...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for ATM B2.");
    info!(log, "Total Duration for ATM B2: {:?}", total_duration);
    println!(
        "Total Duration for ATM B2 .cf file generator: {:2?}",
        total_duration
    );
}
