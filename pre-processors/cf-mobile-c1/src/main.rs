...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Mobile C1.");
    info!(log, "Total Duration for Mobile C1: {:?}", total_duration);
    println!(
        "Total Duration for Mobile C1 .cf file generator: {:2?}",
        total_duration
    );
}
