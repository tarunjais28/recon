...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Mobile C2.");
    info!(log, "Total Duration for Mobile C2: {:?}", total_duration);
    println!(
        "Total Duration for Mobile C2 .cf file generator: {:2?}",
        total_duration
    );
}
