...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for GL.");
    info!(log, "Total Duration for GL: {:?}", total_duration);
    println!(
        "Total Duration for GL .cf file generator: {:2?}",
        total_duration
    );
}
