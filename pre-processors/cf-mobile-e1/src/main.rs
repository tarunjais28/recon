...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for Banking E1.");
    info!(log, "Total Duration for Banking E1: {:?}", total_duration);
    println!(
        "Total Duration for Banking E1 .cf file generator: {:2?}",
        total_duration
    );
}
