...
fn main() {
    ...
    let total_duration = end_main_timer
        ...
        .expect("Could not calculate total duration for ATM B1.");
    info!(log, "Total Duration for ATM B1: {:?}", total_duration);
    println!(
        "Total Duration for ATM B1 .cf file generator: {:2?}",
        total_duration
    );
}
