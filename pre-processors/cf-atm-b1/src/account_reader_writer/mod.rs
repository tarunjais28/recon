    ...
    if let Some(Ok(reader)) = input_file.worksheet_range(config_params.input_sheet_name()) {
        for row in reader.rows() {
            ...
            if tot_acc_encntrd == 1 {
                ...
                log_debug!(log, "Processed: `{}`", input_account.print_header());
            }
            log_debug!(log, "Processed: `{}`", input_account.print());
            if input_account.amount == 0.0 {
                op_line.push_str(&input_account.print());
                continue;
            }
            succ_rec += 1;
            tot_prin_in_inp += input_account.amount;
            ...
            log_measurements!(
                ...
                [format!(
                    ...
                    account_without_cashflows.acc_no
                )],
