...
pub fn generate(config_params: &ConfigurationParameters, log: &Logger, diag_log: &Logger) {
    ...
    let mut op_line: String =
        String::from("datetime|ref_no|crd_no|acc|txn|disp|extra|resp|status\n");
    ...
    if let Some(Ok(reader)) = input_file.worksheet_range(config_params.input_sheet_name()) {
        for row in reader.rows().skip(1) {
            ...
            let account_without_cashflows = log_measurements!(
                ...
                create_account_without_cashflows(input_account.clone())
            );
            if account_without_cashflows.amount == 0.0 {
                op_line.push_str(&input_account.print());
                continue;
            }
            succ_rec += 1;
            tot_prin_in_inp = account_without_cashflows.amount;
            tot_prin_in_op = account_without_cashflows.amount;
            log_measurements!(
                diag_log,
                [format!(
                    "Type: WriteAccWithCFs, Identifier: {}",
                    account_without_cashflows.acc
                )],
